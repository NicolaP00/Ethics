...
def create_explanations(model, X, preprocessor):
    ...
    # Add feature names
    explainer = shap.Explainer(model['classifier'].best_estimator_, X_preprocessed)
...
def summaryPlot(model, X, preprocessor, lf, output_folder, save_name, plot_type):
    ...
    # Create plot 
    fig, ax = plt.subplots()
    shap.summary_plot(explanations, X_preprocessed, show=False, auto_size_plot=True, plot_type=plot_type, max_display=len(lf), sort=False) # TOLTO feature_names = lf
    plt.tight_layout()
...
def Decision_plot(model, X, preprocessor, output_folder, num_example, save_name, lf):
    ...
    # Create plot
    fig, ax = plt.subplots() 
    shap.plots.decision(explainer.expected_value, explanation,show=False, feature_display_range = range(len(lf))) # TOLTO feature_names = lf
    ax.set_title(f"Example {num_example}")
    plt.tight_layout()
    plt.savefig(output_folder + save_name)
    plt.close()
